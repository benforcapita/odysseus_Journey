...
class _RevalidatingStatic(StaticFiles):
    ...
    async def get_response(self, path, scope):
        ...
        # Let the service worker control the whole origin, not just /static/.
        # Without this header, a SW registered at /static/sw.js with scope "/"
        # is rejected by the browser because the script path (/static/sw.js)
        # is outside its declared scope (/).
        if path == "sw.js":
...
async def _startup_event():
    ...
    # Hourly expiration of unsaved tool-run metadata (24-hour TTL).
    async def _tool_run_expiry_loop():
        while True:
            await asyncio.sleep(3600)
            try:
                from core.database import SessionLocal as _SL
                from src.tools_platform.history import expire_temporary_runs
                _db = _SL()
                try:
                    await asyncio.to_thread(expire_temporary_runs, _db, ttl_hours=24)
                finally:
                    _db.close()
            except Exception:
                pass

    _startup_tasks.append(asyncio.create_task(_tool_run_expiry_loop()))

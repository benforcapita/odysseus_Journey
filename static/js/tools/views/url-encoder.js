import { encode, decode } from '../operations/url-encoder.js';
...
export async function render({ fetchImpl, onStatusChange }) {
  ...
  const ta = document.createElement('textarea'); ta.className = 'tool-input-area'; ta.placeholder = 'Enter text to URL-encode/decode...'; ta.rows = 10; ta.setAttribute('aria-label', 'Input'); root.appendChild(ta);
  ...
  async function run(fn, opName) {
    ...
    try {
      const { run, artifact } = await executeTool({ toolId: 'url-encoder', owner: 'default', operationFn: (d) => fn(d), input: text, settings: { operation: opName }, fetchImpl });
